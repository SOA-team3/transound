...
describe 'Test Git Commands Mapper and Gateway' do
  ...
  before do
    ...
    project = TranSound::Repository::For.entity(gh_project)
      .create(gh_project)

    @gitrepo = TranSound::GitRepo.new(project)
  ...
  it 'HAPPY: should get contributions summary for entire repo' do
    root = TranSound::Mapper::Contributions.new(@gitrepo).for_folder('')
  ...
  it 'HAPPY: should get accurate contributions summary for specific folder' do
    forms = TranSound::Mapper::Contributions.new(@gitrepo).for_folder('forms')
